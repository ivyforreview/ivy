...
{
    class Node;
    const int IvyPIFO::ROOT = 233;
    const int IvyPIFO::SIZE = ((1 << 10) - 1) / 3;
    
    NS_OBJECT_ENSURE_REGISTERED(IvyPIFO);
    TypeId IvyPIFO::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::IvyPIFO")
			.SetParent<QueueDisc>()
			.SetGroupName("TrafficControl")
			.AddConstructor<IvyPIFO>();
		return tid;
	}

    bool IvyPIFO::IsHadoop(int x)
    {
        return (x % 10) > (x / 10);
    }
    ...
    std::string IvyPIFO::GetFlowLabel(Ptr<QueueDiscItem> item)
    {
    ...
    bool IvyPIFO::DoEnqueue(Ptr<QueueDiscItem> item)
    {
        TenantTag my_tag;
    ...
    Ptr<QueueDiscItem> IvyPIFO::DoDequeue()
    {
    ...
    Ptr<const QueueDiscItem> IvyPIFO::DoPeek(void) const {
        return 0;
    }
    
    bool IvyPIFO::CheckConfig(void) {
        return 1;
    }
