#include "IvyPIFO_rr.h"

namespace ns3
{
    class Node;
    const int IvyPIFO_rr::ROOT = 233;
    const int IvyPIFO_rr::SIZE = ((1 << 12) - 1) / 3;
    
    NS_OBJECT_ENSURE_REGISTERED(IvyPIFO_rr);
    TypeId IvyPIFO_rr::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::IvyPIFO_rr")
			.SetParent<QueueDisc>()
			.SetGroupName("TrafficControl")
			.AddConstructor<IvyPIFO_rr>();
		return tid;
	}

    bool IvyPIFO_rr::IsHadoop(int x)
    {
        return (x % 10) > (x / 10);
    }

    IvyPIFO_rr::IvyPIFO_rr()
    {
        flow_map.clear();
        sch_tree.clear();
        queue_map.clear();
    ...
    std::string IvyPIFO_rr::GetFlowLabel(Ptr<QueueDiscItem> item)
    {
    ...
    bool IvyPIFO_rr::DoEnqueue(Ptr<QueueDiscItem> item)
    {
        TenantTag my_tag;
    ...
    Ptr<QueueDiscItem> IvyPIFO_rr::DoDequeue()
    {
    ...
    Ptr<const QueueDiscItem> IvyPIFO_rr::DoPeek(void) const {
        return 0;
    }
    
    bool IvyPIFO_rr::CheckConfig(void) {
        return 1;
    }
