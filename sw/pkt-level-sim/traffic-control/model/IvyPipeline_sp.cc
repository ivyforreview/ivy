# include "IvyPipeline_sp.h"

namespace ns3 {
    int IvyPipeline_sp::max_tree_size_t=0;
    int IvyPipeline_sp::max_tree_size_e[T]={0};
    int IvyPipeline_sp::sch_num=0;
    IvyPipeline_sp::IvyPipeline_sp() {
        for (int i = 0; i < N; ++i)
        ...
        Simulator::Schedule(Seconds(START_TIME), &IvyPipeline_sp::WakeUp, this);
    }

    int IvyPipeline_sp::GetListId (int user) {
    ...
    void IvyPipeline_sp::AddPush (int user, long long rank, int token) {
        if (user == ROOT) {
            root_num++;
        }
    ...
    void IvyPipeline_sp::AddPop (int user) {
        if (user == ROOT) {
            root_num--;
            start_num++;
        }
    ...
    void IvyPipeline_sp::SendOperation (int list_id,bool is_push) {
        if (is_push)
    ...
    bool IvyPipeline_sp::CheckRealQueue(int id) {
    ...
    void IvyPipeline_sp::WakeUp () {
        ...
        if (empty_count < END_THRESHOLD)
            Simulator::Schedule(NanoSeconds(CYCLE), &IvyPipeline_sp::WakeUp, this);
        else
            {
                ofstream scht("IvyspResult/Scht.txt", std::ios::out | std::ios::app);
