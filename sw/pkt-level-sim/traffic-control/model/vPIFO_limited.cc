#include "vPIFO_limited.h"

namespace ns3
{
    class Node;
    const int vPIFO_limited::ROOT = 233;
    const int vPIFO_limited::SIZE = ((1 << 10) - 1) / 3;
    
    NS_OBJECT_ENSURE_REGISTERED(vPIFO_limited);
    TypeId vPIFO_limited::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::vPIFO_limited")
			.SetParent<QueueDisc>()
			.SetGroupName("TrafficControl")
			.AddConstructor<vPIFO_limited>();
		return tid;
	}

    bool vPIFO_limited::IsHadoop(int x)
    {
        return (x % 10) > (x / 10);
    }

    vPIFO_limited::vPIFO_limited()
    {
        flow_map.clear();
        sch_tree.clear();
        queue_map.clear();
        ...
        // Layer 1: tenant group to tenant
        for (int i = 0; i <= 9; ++i)
            // sch_tree[i] = std::make_shared<SP>();
            // j=10 + i * 2;
            sch_tree[i] = std::make_shared<WFQ>(10);
        ...
        // Layer 2:
        // sch_tree[10] = std::make_shared<WFQ>(55);
        // sch_tree[11] = std::make_shared<WFQ>(45);
    ...
    std::string vPIFO_limited::GetFlowLabel(Ptr<QueueDiscItem> item)
    {
    ...
    bool vPIFO_limited::DoEnqueue(Ptr<QueueDiscItem> item)
    {
        TenantTag my_tag;
        ...
        // int type_id = 10+IsHadoop(tenant);
        // long long rk1 = sch_tree[ROOT]->RankComputation(type_id, 0);
        // pipe.AddPush(ROOT, rk1, type_id);
        ...
        long long rk3 = sch_tree[group]->RankComputation(
            tenant_id, packet_size);
        pipe.AddPush(group, rk3, tenant_id);
    ...
    Ptr<QueueDiscItem> vPIFO_limited::DoDequeue()
    {
        ...
        // int type_id = 10 + IsHadoop(tenant);
        int group_id=tenant/10;
        sch_tree[group_id]->Dequeue(packet_size);
        if (!IsHadoop(tenant))
    ...
    Ptr<const QueueDiscItem> vPIFO_limited::DoPeek(void) const {
        return 0;
    }
    
    bool vPIFO_limited::CheckConfig(void) {
        return 1;
    }
