#include "IvyPIFO_sp.h"

namespace ns3
{
    class Node;
    const int IvyPIFO_sp::ROOT = 233;
    const int IvyPIFO_sp::SIZE = ((1 << 12) - 1) / 3;
    
    NS_OBJECT_ENSURE_REGISTERED(IvyPIFO_sp);
    TypeId IvyPIFO_sp::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::IvyPIFO_sp")
			.SetParent<QueueDisc>()
			.SetGroupName("TrafficControl")
			.AddConstructor<IvyPIFO_sp>();
		return tid;
	}

    bool IvyPIFO_sp::IsHadoop(int x)
    {
        return (x % 10) > (x / 10);
    }

    IvyPIFO_sp::IvyPIFO_sp()
    {
        flow_map.clear();
        sch_tree.clear();
        queue_map.clear();
    ...
    std::string IvyPIFO_sp::GetFlowLabel(Ptr<QueueDiscItem> item)
    {
    ...
    bool IvyPIFO_sp::DoEnqueue(Ptr<QueueDiscItem> item)
    {
        TenantTag my_tag;
    ...
    Ptr<QueueDiscItem> IvyPIFO_sp::DoDequeue()
    {
    ...
    Ptr<const QueueDiscItem> IvyPIFO_sp::DoPeek(void) const {
        return 0;
    }
    
    bool IvyPIFO_sp::CheckConfig(void) {
        return 1;
    }
