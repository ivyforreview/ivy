...
NS_LOG_COMPONENT_DEFINE("lmTopoTCP");
...
public:
  ...
  {
    if (flow_type == WebSearch) {
      string path = "vPIFOlmResult/flowlabel.txt";
      Output.open(path);
...
} web_search("scratch/traffic_ws_025.txt", TraceFlow::WebSearch), 
  hadoop("scratch/traffic_hd_025.txt", TraceFlow::Hadoop);
...
int main(int argc, char *argv[])
{
  ...
  Config::SetDefault("ns3::TcpSocket::InitialCwnd",UintegerValue(10000));
  ...
  tch.SetRootQueueDisc("ns3::vPIFO_limited");
  for (int i = 0; i < LINK_CNT; ++i)
  {
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = 
    stats.begin (); i != stats.end (); ++i) {
      ...
      cout << "idx=" << idx
     << " rxPkts=" << i->second.rxPackets
     << " lostPkts=" << i->second.lostPackets
     << " txPkts=" << i->second.txPackets
  ...
  ofstream ss_fct("vPIFOlmResult/AllFct.txt", std::ios::out | std::ios::app);
  ...
  ofstream ss_efct("vPIFOlmResult/E2EFct.txt", std::ios::out | std::ios::app);
  for (uint32_t i = 0; i < websearch_count; ++i) {
    ss_efct << flow_info[i].e2e_time << " " << flow_info[i].size << " " <<
        flow_info[i].size * 8 * PKTSIZE / (1024 * 1024 * flow_info[i].time) << 
        " " << flow_info[i].tenant << endl; 
  }
  ofstream ss_95("vPIFOlmResult/95Fct.txt", std::ios::out | std::ios::app);
  ...
  ofstream ss_e95("vPIFOlmResult/E2E95Fct.txt", std::ios::out | std::ios::app);
  for (uint32_t i = 0, j, k; i < websearch_count;) {
    j = i;
    while (flow_info[j].tenant == flow_info[i].tenant)
      j++;
