...
void MyApp::SendPacket(void)
{
  ...
  int actual_sent = m_socket->Send(packet);    
  if (actual_sent > 0){
    total_send += 1;
    m_packetsSent++;
  }
...
int main(int argc, char *argv[])
{
  ...
  Config::SetDefault("ns3::TcpSocketBase::MinRto",
                     TimeValue(MilliSeconds(1e6)));//TimeValue(Seconds(0.000060))); // 3RTT=36us
  ...
  ofstream ss_efct("vPIFOResult/E2EFct.txt", std::ios::out | std::ios::app);
  for (uint32_t i = 0; i < websearch_count; ++i) {
  ...
  ofstream ss_e95("vPIFOResult/E2E95Fct.txt", std::ios::out | std::ios::app);
