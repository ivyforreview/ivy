...
NS_LOG_COMPONENT_DEFINE("IvyrrTopoTCP");
...
public:
  ...
  {
    if (flow_type == WebSearch) {
      string path = "IvyrrResult/flowlabel.txt";
      Output.open(path);
...
} web_search("scratch/traffic_ws_01.txt", TraceFlow::WebSearch), 
  hadoop("scratch/traffic_hd_01.txt", TraceFlow::Hadoop);
...
int main(int argc, char *argv[])
{
  ...
  tch.SetRootQueueDisc("ns3::IvyPIFO_rr");
  for (int i = 0; i < LINK_CNT; ++i)
  {
  ...
  ofstream ss_fct("IvyrrResult/AllFct.txt", std::ios::out | std::ios::app);
  ...
  ofstream ss_efct("IvyrrResult/E2EFct.txt", std::ios::out | std::ios::app);
  for (uint32_t i = 0; i < websearch_count; ++i) {
    ss_efct << flow_info[i].e2e_time << " " << flow_info[i].size << " " <<
        flow_info[i].size * 8 * PKTSIZE / (1024 * 1024 * flow_info[i].time) << 
        " " << flow_info[i].tenant << endl; 
  }
  ofstream ss_95("IvyrrResult/95Fct.txt", std::ios::out | std::ios::app);
  ...
  ofstream ss_e95("IvyrrResult/E2E95Fct.txt", std::ios::out | std::ios::app);
  for (uint32_t i = 0, j, k; i < websearch_count;) {
    j = i;
    while (flow_info[j].tenant == flow_info[i].tenant)
      j++;
