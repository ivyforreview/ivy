        ...
        for i in range(0, self.n):
            ...
            if new_ins['val']>0:
                ...
                if poe==0 and pue==0:
                    ...
                    self.ppl_add(t+1,i,'push',self._parse_instr(self.push_queues[i][0])[0],random.randint(self.n,self.n+1),self._parse_instr(self.push_queues[i][0])[1])
                    self.push_queues[i].pop(0)
                    self.rr_flag[i]=True
                    self.pfirst_flag[i]=1
                elif pue==0:
                    self.ppl_add(t+1,i,'pop',self._parse_instr(self.pop_queues[i][0])[0],random.randint(self.n,self.n+1),self._parse_instr(self.pop_queues[i][0])[1])
