...
fill_tree0=[2,4,8,16,32,64]
fill_tree1=[2,2,4,8,16,32]
fill_tree2=[2,2,4,8,16]
fill_tree3=[2,2,4,8]
...
def fill_tree_num(fill_tree,mem):
    ...
    while flag:
        for i in range(len(fill_tree)):
            temp_mem[(num+i)%6]-=fill_tree[i]
            if temp_mem[(num+i)%6]<0:
                flag=False
                break
        if flag:
            num+=1
...
if __name__ == '__main__':
    ...
    while cur_mem_size<=1024*8:
        mem_sizes.append(cur_mem_size * 6 / 1024)

        mem=[cur_mem_size]*mem_num
        num_0.append(float(fill_tree_num(fill_tree0,mem)*126/1024))
        num_2.append(float(fill_tree_num(fill_tree2,mem)*32/1024))
        num_1.append(float(fill_tree_num(fill_tree1,mem)*64/1024))
        num_3.append(float(fill_tree_num(fill_tree3,mem)*16/1024))
